...
class UserManagementController extends Controller
{
    ...
    public function getUsersData(Request $request)
    {
        ...
        foreach ($records as $key => $record) {
            $record->name = '<h2 class="table-avatar"><a href="#" class="name">'.'<img class="avatar" data-avatar='.$record->avatar.' src="'.url('/assets/images/'.$record->avatar).'">' .$record->name.'</a></h2>';
            if ($record->role_name == 'Admin') { /** color role name */
                $role_name = '<span class="badge bg-inverse-danger role_name">'.$record->role_name.'</span>';
            }  elseif ($record->role_name == 'Normal Usuario') {
                $role_name = '<span class="badge bg-inverse-info role_name">'.$record->role_name.'</span>';
            } elseif ($record->role_name == 'Auxiliar Temporal') {
    ...
    public function addNewUserSave(Request $request)
    {
        ...
        try{
            ...
            $user->save();
            DB::commit();
            Toastr::success('Cuenta de usuario creada :)','Success');
            return redirect()->route('userManagement');
        }catch(\Exception $e){
            DB::rollback();
            Toastr::error('FALLO de creacion de la cuenta :)','Error');
            return redirect()->back();
        }
    }
